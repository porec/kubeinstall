...
export PCC_FILE=$(echo $PCC_DL_LNK | cut -d "/" -f 6)

echo "------------------------------------------------------------------------------------------------------------------------------------"
echo "${green}Insert Prisma Cloud Compute Access Token${reset}"
echo "------------------------------------------------------------------------------------------------------------------------------------"
read pcc_tok
export PCC_TOKEN=$acc_tok
...
export PCC_CIP=$(kubectl get pod -A -o wide | grep etcd-master | cut -c101-117 | tr -d " ")
export PCC_SIP=$(kubectl get services -A | grep twistlock-console | cut -c86-100 | tr -d " ")
